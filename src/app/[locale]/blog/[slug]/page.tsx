...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://vaga.tn/${locale}/blog/${slug}`,
      languages: {
        "fr": `https://vaga.tn/fr/blog/${slug}`,
        "en": `https://vaga.tn/en/blog/${slug}`,
        "x-default": `https://vaga.tn/fr/blog/${slug}`,
      },
    },
    openGraph: {
      title: title as string,
      description: excerpt as string | undefined,
      url: `https://vaga.tn/${locale}/blog/${slug}`,
...
const richTextOptions: Options = {
  ...
  renderNode: {
    ...
    [BLOCKS.HEADING_2]: (_, children) => (
      <h2 className="mt-10 mb-4 text-2xl font-bold tracking-tight text-zinc-900">
        {children}
      </h2>
    ),
    [BLOCKS.HEADING_3]: (_, children) => (
      <h3 className="mt-8 mb-3 text-xl font-semibold text-zinc-900">
        {children}
      </h3>
    ),
    [BLOCKS.HEADING_4]: (_, children) => (
      <h4 className="mt-6 mb-2 text-lg font-semibold text-zinc-900">
        {children}
      </h4>
    ),
    ...
    [BLOCKS.TABLE_HEADER_CELL]: (_, children) => (
      <th className="bg-zinc-50 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-500 border-b border-zinc-200 whitespace-nowrap">
        {children}
      </th>
    ),
      ...
      <article className="container max-w-3xl px-4 py-16 mx-auto lg:px-0">
        ...
        {publishedDate && (
          <time className="block mb-10 text-sm text-zinc-400">
            {new Date(publishedDate as string).toLocaleDateString(
              locale === "fr" ? "fr-TN" : "en-GB",
        ...
        <div className="prose-zinc">
          {body &&
            documentToReactComponents(
              body as Parameters<typeof documentToReactComponents>[0],
              richTextOptions
            )}
        </div>
      </article>
