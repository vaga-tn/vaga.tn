...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://vaga.tn/${locale}/changelog`,
      languages: {
        "fr": "https://vaga.tn/fr/changelog",
        "en": "https://vaga.tn/en/changelog",
        "x-default": "https://vaga.tn/fr/changelog",
      },
    },
  }
}
    ...
    <main className="min-h-screen font-sans bg-background text-zinc-900 scroll-smooth">
      ...
      <div className="relative border-b sectionWrapper border-zinc-400">
        <div className="container px-4 mx-auto lg:px-0">
          ...
          <p className="max-w-2xl text-lg text-zinc-600">{t("subtitle")}</p>
        </div>
        <Image
          src="/fennec_changelog.webp"
          alt="VAGA"
          width={154}
          height={154}
          className="absolute bottom-0 right-0 z-10 object-contain opacity-50 lg:opacity-100"
        />
      </div>
        ...
        <div className="container max-w-6xl px-4 mx-auto">
          <div className="relative ml-4 border-l border-zinc-200 md:ml-12 lg:ml-24">
            {changelogData.map((item) => (
              <div
                key={item.id}
                className="relative flex flex-col gap-6 pl-8 mb-24 last:mb-0 md:pl-16 md:flex-row md:gap-16"
              >
                      ...
                      <ul className="space-y-2 text-sm text-zinc-600">
                        {item.linksSection.links.map((link, i) => (
                          <li key={i}>
                            ...
                            <a
                              href={link.url}
                              className="underline transition-colors underline-offset-4 decoration-zinc-300 hover:decoration-zinc-900 text-zinc-900"
                            >
                              {link.url}
                            </a>
                          </li>
